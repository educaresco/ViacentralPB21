$PBExportHeader$w_groupvscountry_agencyfee.srw
forward
global type w_groupvscountry_agencyfee from Window
end type
type cb_3 from commandbutton within w_groupvscountry_agencyfee
end type
type cb_2 from commandbutton within w_groupvscountry_agencyfee
end type
type cb_1 from commandbutton within w_groupvscountry_agencyfee
end type
type dw_1 from datawindow within w_groupvscountry_agencyfee
end type
type em_fecha2 from editmask within w_groupvscountry_agencyfee
end type
type em_fecha1 from editmask within w_groupvscountry_agencyfee
end type
type st_2 from statictext within w_groupvscountry_agencyfee
end type
type st_1 from statictext within w_groupvscountry_agencyfee
end type
end forward
...
global w_groupvscountry_agencyfee w_groupvscountry_agencyfee

on w_groupvscountry_agencyfee.create
...
on w_groupvscountry_agencyfee.destroy
destroy(this.cb_3)
destroy(this.cb_2)
destroy(this.cb_1)
destroy(this.dw_1)
...
event clicked;close(w_groupvscountry_agencyfee)
end event
...
MESSAGEBOX("File Name Is c:\dinero\files\GCAF.txt","OK")

ls_file = 'c:\dinero\files\GCAF.txt'
li_FileNum = FileOpen(ls_file,LineMode!, Write!, LockWrite!, Replace!)
ls_cadena = 'AGENCY FEES GROUP VS COUNTRY AUMENTAR UNA LINEA PARA LA 0213'
...
ls_cadena = 'Name'+','+'Name/Group'+','+'Colombia'+','+'C.Rica'+','+'Ecuador'+','+'El Salvador'+','+'Honduras'+','+'Brasil'+','+'Mexico'+','+'Nicaragua'+','+'Peru'+','+'R.D.'+','+'USA'+','+'Other'+','+'Total'
...
DO WHILE SQLCA.SQLCODE = 0
	...
	SELECT sum(dba.receiver.telex_company + dba.receiver.exchange_company + dba.receiver.total_modo_pago_comp)
   INTO :ll_positivo
   ...
   WHERE ( dba.branch.id_branch = dba.receiver.id_branch ) and  
         ( (dba.branch.id_main_branch = :ls_group ) AND  
         ( dba.receiver.id_flag_receiver <> 'A' ) AND  
         ( dba.receiver.id_country_receiver IN ('RD') ) AND  
         ( dba.receiver.date_receiver >= :ld_fecha1 ) AND  
         ( dba.receiver.date_receiver <= :ld_fecha2 ) )   ;
   IF isnull(ll_positivo) then
	   ll_positivo = 0
...
int Height=1848
int TabOrder=50
string DataObject="dw_grupovscountry_agencyfee"
