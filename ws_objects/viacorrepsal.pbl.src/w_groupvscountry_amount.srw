$PBExportHeader$w_groupvscountry_amount.srw
forward
global type w_groupvscountry_amount from Window
end type
type cb_3 from commandbutton within w_groupvscountry_amount
end type
type cb_2 from commandbutton within w_groupvscountry_amount
end type
type cb_1 from commandbutton within w_groupvscountry_amount
end type
type dw_1 from datawindow within w_groupvscountry_amount
end type
type em_fecha2 from editmask within w_groupvscountry_amount
end type
type em_fecha1 from editmask within w_groupvscountry_amount
end type
type st_2 from statictext within w_groupvscountry_amount
end type
type st_1 from statictext within w_groupvscountry_amount
end type
end forward
...
global w_groupvscountry_amount w_groupvscountry_amount

on w_groupvscountry_amount.create
...
on w_groupvscountry_amount.destroy
destroy(this.cb_3)
destroy(this.cb_2)
destroy(this.cb_1)
destroy(this.dw_1)
...
event clicked;close(w_groupvscountry_amount)
end event
...
MESSAGEBOX("File Name Is c:\GCTOTAL.txt","OK")

ls_file = 'c:\GCTOTA.txt'
li_FileNum = FileOpen(ls_file,LineMode!, Write!, LockWrite!, Replace!)
ls_cadena = 'TOTALS GROUP VS COUNTRY'
...
DO WHILE SQLCA.SQLCODE = 0
   ...
   WHERE ( dba.branch.id_branch = dba.receiver.id_branch ) and  
         ( (dba.branch.id_main_branch = :ls_group ) AND  
         ( dba.receiver.id_flag_receiver <> 'A' ) AND  
         ( dba.receiver.id_country_receiver IN ('PER','PED','PDB') ) AND  
         ( dba.receiver.date_receiver >= :ld_fecha1 ) AND  
         ( dba.receiver.date_receiver <= :ld_fecha2 ))    ;
   IF isnull(ll_positivo) then
	   ll_positivo = 0
   ...
   WHERE ( dba.branch.id_branch = dba.receiver.id_branch ) and  
         ( (dba.branch.id_main_branch = :ls_group ) AND  
         ( dba.receiver.id_flag_receiver NOT IN ('MEX' ,'MBN','BAN','SEFI',,'MXD','NI','NID','HON','HOD','COL','BRA','BRD','CSR','CRP','RD','ELS','ELD','ESF','ELR','USA','US2','PER','PED','PDB','ECU','PIC','ECC')) AND
         ( dba.receiver.date_receiver >= :ld_fecha1 ) AND  
         ( dba.receiver.date_receiver <= :ld_fecha2 )  )  ;
   IF isnull(ll_positivo) then
	   ll_positivo = 0
