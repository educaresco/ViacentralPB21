$PBExportHeader$w_despliegue_transacciones_comm.srw
forward
global type w_despliegue_transacciones_comm from window
end type
type cb_4 from commandbutton within w_despliegue_transacciones_comm
end type
type cb_fax from commandbutton within w_despliegue_transacciones_comm
end type
type lb_fax from listbox within w_despliegue_transacciones_comm
end type
type cb_3 from commandbutton within w_despliegue_transacciones_comm
end type
type sle_des2 from singlelineedit within w_despliegue_transacciones_comm
end type
type sle_des1 from singlelineedit within w_despliegue_transacciones_comm
end type
type em_1 from editmask within w_despliegue_transacciones_comm
end type
type rb_1 from radiobutton within w_despliegue_transacciones_comm
end type
type cb_conciliar from commandbutton within w_despliegue_transacciones_comm
end type
type cb_2 from commandbutton within w_despliegue_transacciones_comm
end type
type rb_acoount from radiobutton within w_despliegue_transacciones_comm
end type
type rb_date from radiobutton within w_despliegue_transacciones_comm
end type
type dw_2 from datawindow within w_despliegue_transacciones_comm
end type
type cb_print from commandbutton within w_despliegue_transacciones_comm
end type
type cb_select from commandbutton within w_despliegue_transacciones_comm
end type
type cb_1 from commandbutton within w_despliegue_transacciones_comm
end type
type dw_1 from u_dw within w_despliegue_transacciones_comm
end type
type gb_1 from groupbox within w_despliegue_transacciones_comm
end type
end forward
...
string title = "ACCOUNT - COMMISSION TRANSACTIONS"
...
global w_despliegue_transacciones_comm w_despliegue_transacciones_comm
...
on w_despliegue_transacciones_comm.create
...
on w_despliegue_transacciones_comm.destroy
destroy(this.cb_4)
destroy(this.cb_fax)
destroy(this.lb_fax)
destroy(this.cb_3)
...
if ll_row <> 0 then
	ls_concepto_contable = TRIM(dw_1.getitemstring(ll_row,"concepto_contable"))
	
	if ls_concepto_contable = "TDC" Then
		ld_referencia = dw_1.getitemnumber(ll_row,"cons")
		Openwithparm(w_despliegue_transacciones_detalle_comm,ld_referencia)
		Return
	end if
...
integer height = 1692
integer taborder = 0
string dataobject = "dw_despliegue_transacciones_comm"
   ...
   This.SelectRow(row, TRUE)
	...
	if row <> 0 then
		id_cons     = dw_1.getitemnumber(row,"cons")
	  	SELECT 	dba.TRANSACCION_DIARIA_PAYEE_COMM.DES_TRANS_DIARIA//,dba.TRANSACCION_DIARIA_PAYEE_COMM.DESC_TRANS_DIARIA1  
     	INTO 		:ls_desc1//,:ls_desc2  
     	FROM 		dba.TRANSACCION_DIARIA_PAYEE_COMM  
     	WHERE 	dba.TRANSACCION_DIARIA_PAYEE_COMM.CONS_TRANS_DIARIA = :id_cons   ;
