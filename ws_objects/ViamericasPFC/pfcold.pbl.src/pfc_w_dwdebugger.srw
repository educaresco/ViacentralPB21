$PBExportHeader$pfc_w_dwdebugger.srw
...
If IsNull(is_current_column) Or Len(is_current_column) = 0 Then Return True
...
//	ddlb_columnstatus.enabled = true
	...
	if ll_row > 0 And Len(is_current_column) > 0 then 
...
li_end_posi = 1
do 
	li_end_posi = pos(ls_attr_list,'~t',li_start_posi)
	if li_end_posi = 0 then 
		li_end_posi = len(ls_attr_list) + 1
		lb_done = true
	end if
	ls_attr = mid(ls_attr_list,li_start_posi,li_end_posi - li_start_posi)
	ls_dw_rqst = ''
	if as_object <> 'datawindow' then
		...
		if ls_attr <> 'attributes' and (pos(ls_attr,'data') = 0) and (pos(ls_attr,'syntax') = 0) and ls_attr <> 'objects' then // do not ask for attributes again or data attributes
			ls_dw_rqst = as_object+'.'+ls_attr
		end if
	end if
	if ls_dw_rqst <> '' then
