$PBExportHeader$n_cst_carga_plano_banco_chevy.sru
forward
global type n_cst_carga_plano_banco_chevy from nonvisualobject
end type
end forward

global type n_cst_carga_plano_banco_chevy from nonvisualobject
end type
global n_cst_carga_plano_banco_chevy n_cst_carga_plano_banco_chevy
...
FOR li_i=1 TO adw_archivo_plano.RowCount()
	...
	//JCMONTOYA
	//Debido a que este banco importa con "Enter", el import file del datawindow queda cargado conuna fila nula y cuendo va a realizar el insert
	//genera errorer, por esos se condiciona por la fecha, debido a que esta no puede llegar nula.
	
	IF NOT ISNULL(ls_fecha_transaccion) THEN 
		...
		ldt_DATE_TRANSACTION = of_set_date_transaction (ls_fecha_transaccion)
			...
			THROW CREATE c_sqlexception
				CATCH (c_sqlexception sql1)
					...
					END IF
				END TRY
	END IF//Fin de condicional de la fecha
...
on n_cst_carga_plano_banco_chevy.create
call super::create
TriggerEvent( this, "constructor" )
end on

on n_cst_carga_plano_banco_chevy.destroy
